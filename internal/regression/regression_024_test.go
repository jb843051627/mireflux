...
func readyCycle24(t testing.TB, lab *service.Lab, ctx context.Context) (model.Campaign, model.Chamber, model.SamplingCycle) {
	t.Helper()
	now := time.Now().UTC()
	campaign, err := lab.CreateCampaign(ctx, model.CreateCampaignInput{
		ID: "campaign-24", Name: "Fen transect", Peatland: "north fen", Operator: "field-team",
	...
	chamber, err := lab.RegisterChamber(ctx, model.RegisterChamberInput{
		ID: "chamber-24", CampaignID: campaign.ID, Label: "north collar", Plot: "P-7", VolumeL: 20,
	})
	if err != nil {
	...
	cycle, err := lab.StartCycle(ctx, model.StartCycleInput{
		ID: "cycle-24", CampaignID: campaign.ID, ChamberID: chamber.ID, Sequence: 1,
	})
	if err != nil {
		t.Fatalf("start cycle: %v", err)
	}
	if _, err := lab.RecordCalibration(ctx, model.RecordCalibrationInput{
		ID: "calibration-24", ChamberID: chamber.ID, Instrument: "LI-7810", OffsetPPM: 5, SpanFactor: 1.02,
	...
	for index, co2 := range []float64{400, 410, 420} {
		if _, err := lab.RecordReading(ctx, model.RecordReadingInput{
			ID: fmt.Sprintf("reading-24-%d", index), CycleID: cycle.ID, ChamberID: chamber.ID,
...
func TestBug24_DailyReportFiltersCalendarYear(t *testing.T) {
	lab, repository := lab24(t)
	target := time.Date(2026, time.January, 2, 12, 0, 0, 0, time.UTC)
	for _, cycle := range []model.SamplingCycle{
		{ID: "target", CampaignID: "campaign", ChamberID: "chamber", StartedAt: target, State: model.CycleOpen, Sequence: 1},
		{ID: "prior-year", CampaignID: "campaign", ChamberID: "chamber", StartedAt: target.AddDate(-1, 0, 0), State: model.CycleOpen, Sequence: 2},
	} {
		if err := repository.Save(context.Background(), "cycle", cycle.ID, cycle); err != nil {
			t.Fatalf("save cycle: %v", err)
		}
	}
	report, err := lab.DailyReport(context.Background(), "campaign", target)
	if err != nil {
		t.Fatalf("daily report: %v", err)
	}
	if report.CycleCount != 1 {
		t.Fatalf("cycle count = %d, want only target-year cycle", report.CycleCount)
	}
}
